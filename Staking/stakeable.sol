...
*/
 contract Stakeable is Initializable, OwnableUpgradeable, UUPSUpgradeable{

    
    modifier sunset{
        require(block.timestamp < sunsetPeriod,'staking is off');
        _;
    }
    /*
    ...
    * push once to stakeholders for it to work proplerly
     */
    function initialize(address devTokenAddress_, uint256 sunsetPeriod_) external initializer {
        // This push is needed so we avoid index 0 causing bug of index-1
        __Ownable_init();
        __UUPSUpgradeable_init();
        stakeholders.push();
        devTokenAddress = devTokenAddress_;
        rewardPerHour = 1000;
        sunsetPeriod = sunsetPeriod_;
        sunsetToggle=1;
    }
    /*
     * 
    ...
    *  Staked event is triggered whenever a user stakes tokens, address is indexed to make it filterable
    ...
    uint256 public sunsetPeriod;
    uint public sunsetToggle;
    ...
    function stake(uint256 _amount) public {
         ...
         if( sunsetToggle == 1 ){
          ...
          require(_amount < IdevToken(devTokenAddress).balanceOf(msg.sender), "DevToken: Cannot stake more than you own");
      
        _stake(_amount);
                // Burn the amount of tokens on the sender
        IdevToken(devTokenAddress).burn(msg.sender, _amount);
    }
    ...
    function withdrawStake(uint256 amount, uint256 stake_index)  public {
      ...
      IdevToken(devTokenAddress).mint(msg.sender, amount_to_mint);
    }
    
    
    function updateReward(uint256 reward_) external onlyOwner{
        rewardPerHour=reward_;
    }

    
    function updateStakePeriod(uint256 sunsetPeriod_) external onlyOwner{
        sunsetPeriod=sunsetPeriod_;
    }

    // 0 - off , 1 - on
    function toggleSunset(uint toggle_) external onlyOwner{
        require(toggle_== 0 || toggle_== 1,'invalid val');
        sunsetToggle=toggle_;
    }
    /*
